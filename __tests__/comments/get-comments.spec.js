...
import successSchema from 'schemas/comments/get-comments';
...
describe('Get comments', () => {
  ...
  test('successfully', async () => {
    const { status, body, headers } = await request
      .get(
        `solicitations/${Solicitations.id}/evaluateds/${SolicitationsEvaluations.evaluatedId}/comments`,
      )
  ...
  //https://solides.atlassian.net/browse/TDEP-4320
  each`
  ...
  `.test.skip(
    ...
    async ({ id, statusCode, message }) => {
      const { status, body, headers } = await request
        .get(
          `solicitations/${id}/evaluateds/${SolicitationsEvaluations.evaluatedId}/comments`,
        )
        .set('Authorization', `Bearer ${client.accessToken}`);
  ...
  `.test.skip(
    ...
    async ({ id, statusCode, message }) => {
      const { status, body, headers } = await request
        .get(`solicitations/${Solicitations.id}/evaluateds/${id}/comments`)
        .set('Authorization', `Bearer ${client.accessToken}`);
