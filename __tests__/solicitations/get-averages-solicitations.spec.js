...
import successSchema from 'schemas/solicitations/get-averages-solicitations';
import { EXPIRED_TOKEN, UNAUTHORIZED_TOKEN } from 'utils/constants';
import simpleErrorSchema from 'schemas/errors/simple-error';
import errorsSchema from 'schemas/errors/errors';

describe('Get averages Solicitations by id', () => {
  beforeAll(async () => {
    await client.auth();
  ...
  test('successfully', async () => {
    const { status, body, headers } = await request
      .get(`solicitations/${Solicitations.id}/averages`)
